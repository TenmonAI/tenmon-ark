  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* ── 三九法 ── */}
        {/* ============================================================ */}
        {/* §X. 宿曜算出基準について — TENMON_SITE_ALGORITHM_V1           */}
        {/* ============================================================ */}
        ...
        >
          ...
          <p style={{ fontSize: 14, lineHeight: 1.7, color: C.text, margin: 0 }}>
            TENMON-ARKは、<strong>角宿＝1番</strong>の伝統採番、
            <strong>均等分割</strong>（360°÷27）、<strong>JST 0時基準</strong>、
            <strong>太陰太陽暦（旧暦）</strong>で宿を算出しています。
            他サイトと番号が違っても、宿の名前が同じなら算出は一致しています。
          </p>
        </div>

        <p style={{ fontSize: 14, lineHeight: 1.9, color: C.text, marginBottom: 16 }}>
          宿曜占星術には、一つだけの「正解」はありません。
          流派によって採番が異なり、暦の基準が異なり、月が宿を跨ぐ境界時刻の定義も異なります。
          同じ生年月日でも、採用する基準によって算出される宿が一つズレることがあります。
        </p>
        <p style={{ fontSize: 14, lineHeight: 1.9, color: C.text, marginBottom: 24 }}>
          TENMON-ARKは、この事実を隠しません。
          <strong>どの基準を、なぜ採用しているか</strong>を明示することこそが、
          占術システムとしての誠実さだと考えています。
        </p>

        {/* ── 3つの選択軸 ── */}
        <h3 style={{ fontSize: 15, fontWeight: 700, color: C.text, marginTop: 28, marginBottom: 12 }}>
          宿曜算出における3つの選択軸
        </h3>

        {/* 軸1 */}
        <div style={{ background: C.sectionBg, border: `1px solid ${C.sectionBorder}`, borderRadius: 8, padding: 16, marginBottom: 12 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: C.arkGold, marginBottom: 6 }}>軸1 ／ 採番順序</div>
          <div style={{ fontSize: 14, fontWeight: 600, color: C.text, marginBottom: 8 }}>どの宿を「1番」とするか</div>
          <ul style={{ fontSize: 13, lineHeight: 1.8, color: C.text, paddingLeft: 18, margin: 0 }}>
            <li><strong>伝統採番（角宿＝1）</strong> — 空海伝来『宿曜経』原典の順序。古代インド天文学の原型に最も近い</li>
            <li><strong>流派採番（昴宿＝1）</strong> — 日本の近現代占術流派で広く使われる順序</li>
          </ul>
          <p style={{ fontSize: 12, lineHeight: 1.7, color: C.textSub, marginTop: 10, marginBottom: 0 }}>
            どちらも宿そのものの定義は同じです。違うのは「何番目に書くか」だけ。
            斗宿は斗宿、角宿は角宿であり、番号が違うから別の宿になるわけではありません。
          </p>
        </div>

        {/* 軸2 */}
        <div style={{ background: C.sectionBg, border: `1px solid ${C.sectionBorder}`, borderRadius: 8, padding: 16, marginBottom: 12 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: C.arkGold, marginBottom: 6 }}>軸2 ／ 分度法</div>
          <div style={{ fontSize: 14, fontWeight: 600, color: C.text, marginBottom: 8 }}>27宿をどう分割するか</div>
          <ul style={{ fontSize: 13, lineHeight: 1.8, color: C.text, paddingLeft: 18, margin: 0 }}>
            <li><strong>均等分割法</strong> — 月の黄経360°を27等分（各宿13.333°）。再現性が高く、検証可能</li>
            <li><strong>不均等分割法</strong> — 斗宿（牽牛宿）だけ特別な分度を割り当てる古式。流派により分度が異なる</li>
          </ul>
        </div>

        {/* 軸3 */}
        <div style={{ background: C.sectionBg, border: `1px solid ${C.sectionBorder}`, borderRadius: 8, padding: 16, marginBottom: 20 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: C.arkGold, marginBottom: 6 }}>軸3 ／ 日付基準</div>
          <div style={{ fontSize: 14, fontWeight: 600, color: C.text, marginBottom: 8 }}>月の位置をいつ計測するか</div>
          <ul style={{ fontSize: 13, lineHeight: 1.8, color: C.text, paddingLeft: 18, margin: 0 }}>
            <li><strong>JST基準</strong> — 日本標準時0時で宿を判定</li>
            <li><strong>UTC基準</strong> — 協定世界時0時で判定（JSTと9時間差）</li>
            <li><strong>正午基準</strong> — 各地の正午で判定する古式</li>
          </ul>
          <p style={{ fontSize: 12, lineHeight: 1.7, color: C.textSub, marginTop: 10, marginBottom: 0 }}>
            月は27.3日で全天を一周するため、1日に約13°移動します。
            境界日ではタイムゾーンの差で宿が1つズレる可能性があります。
          </p>
        </div>
        ...
        <div style={{ background: C.card, border: `2px solid ${C.arkGoldBorder}`, borderRadius: 8, padding: 16, marginBottom: 20 }}>
          {[
            ...
            <div key={i} style={{ display: "flex", borderBottom: i < 5 ? `1px solid ${C.border}` : "none", padding: "10px 0", gap: 12 }}>
              <div style={{ flex: "0 0 90px", fontSize: 13, fontWeight: 700, color: C.textSub }}>{row.label}</div>
              <div style={{ flex: 1, fontSize: 14, color: C.text, fontWeight: 500 }}>{row.value}</div>
            </div>
          ))}
        </div>

        <h4 style={{ fontSize: 14, fontWeight: 700, color: C.text, marginTop: 20, marginBottom: 8 }}>
          なぜ、この基準なのか
        </h4>
        <ul style={{ fontSize: 13, lineHeight: 1.9, color: C.text, paddingLeft: 18, marginBottom: 20 }}>
          <li><strong>原典『宿曜経』への忠実性</strong> — 空海が伝えた採番は千年以上変わっていません。流派採番は運営者の解釈によって将来変動する可能性があります</li>
          <li><strong>計算の再現性</strong> — 均等分割は誰が計算しても同じ結果になります。不均等法は流派ごとに分度が違い、標準化されていません</li>
          <li><strong>日本の暦との整合性</strong> — 旧暦自体が東アジアの暦法であり、日本時間との整合性が自然です</li>
          <li><strong>宿名ファースト表示</strong> — 宿の本質は名前にあります。番号を前面に出すと、異なる採番体系の他サイトと比較したときにユーザーが混乱します</li>
        </ul>
        ...
        <div style={{ background: C.quoteBg, border: `1px solid ${C.quoteBorder}`, borderLeft: `4px solid ${C.quoteBorder}`, borderRadius: 6, padding: 16, marginBottom: 16 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: C.textSub, marginBottom: 10, letterSpacing: 1 }}>
            例 ／ 1990年9月26日生まれ
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
              <div style={{ flex: "0 0 110px", fontSize: 13, fontWeight: 600, color: C.textSub }}>TENMON-ARK</div>
              <div style={{ fontSize: 14, color: C.text }}>
                <strong>斗宿</strong>
                <span style={{ color: C.textSub, fontSize: 12, marginLeft: 8 }}>（伝統採番 8番）</span>
              </div>
            </div>
            <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
              <div style={{ flex: "0 0 110px", fontSize: 13, fontWeight: 600, color: C.textSub }}>他サイト（昴宿=1系）</div>
              <div style={{ fontSize: 14, color: C.text }}>
                <strong>斗宿</strong>
                <span style={{ color: C.textSub, fontSize: 12, marginLeft: 8 }}>（流派採番 19番）</span>
              </div>
            </div>
          </div>
          <p style={{ fontSize: 13, lineHeight: 1.7, color: C.text, marginTop: 12, marginBottom: 0 }}>
            番号は違いますが、<strong>同じ「斗宿」</strong>です。これは誤りではなく、採番体系が異なるだけです。
          </p>
        </div>

        <p style={{ fontSize: 13, lineHeight: 1.9, color: C.textSub, marginBottom: 20 }}>
        ...
        <p style={{ fontSize: 14, lineHeight: 1.9, color: C.text, marginBottom: 16 }}>
          TENMON-ARKは、算出精度を3段階で管理し、鑑定結果の下部にこのランクを表示しています。
        </p>
        ...
        <div style={{ marginBottom: 20 }}>
          {[
            ...
            <div key={r.rank} style={{ display: "flex", gap: 14, padding: "12px 14px", marginBottom: 8, background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, alignItems: "flex-start" }}>
     [200~cd /opt/tenmon-ark-repo/web

# 挿入するコードを一時ファイルに保存
cat > /tmp/algorithm_section.tsx <<'ALGOEOF'
        {/* ============================================================ */}
        {/* §X. 宿曜算出基準について — TENMON_SITE_ALGORITHM_V1           */}
        {/* ============================================================ */}
        <SectionTitle>宿曜算出基準について</SectionTitle>
        ...
        <p style={{ fontSize: 14, lineHeight: 1.9, color: C.text, marginBottom: 0 }}>
