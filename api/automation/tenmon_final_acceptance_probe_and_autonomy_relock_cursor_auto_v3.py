...
P0〜P4 後の再観測。V1 エンジンを再利用し、プローブ集合は P5 指定（UX + continuity_1..3）。
V2 比で kokuzo 4 key・verdict/autonomy 非後退を acceptance に重ねる（fail-closed）。
成果物: tenmon_final_acceptance_probe_relock_result_v3.json / _report_v3.md
"""
...
# P5 プローブ順（V1 UX_PROBES からこの順で抽出）+ continuity は CONTINUITY_PAIRS で別実行
...
def _patch_result_json(
    ...
    data = json.loads(path.read_text(encoding="utf-8"))
    data["schema"] = "TENMON_FINAL_ACCEPTANCE_PROBE_AND_AUTONOMY_RELOCK_V3"
    data["card"] = CARD
    data["probe_profile"] = "P5_v3_ux_plus_continuity"
    data["nextOnPass"] = "TENMON_ARTIFACT_AND_WORKTREE_HYGIENE_CURSOR_AUTO_V1"
    data["nextOnFail"] = "TENMON_SURFACE_LEAK_CLEANUP_RETRY_CURSOR_AUTO_V5"
    ...
    vf = cur_r > prev_r if prev_r >= 0 and cur_r >= 0 else None
    v_non_regress = cur_r >= prev_r if prev_r >= 0 and cur_r >= 0 else True
    data["verdict_comparison"] = {
        ...
        "verdict_forward": vf,
        "verdict_non_regress": v_non_regress,
    }
    ...
    base_pass = bool(data.get("acceptance_pass"))
    v3_kok = bool(kokuzo.get("all_four_present"))
    v3_verdict = bool(data["verdict_comparison"].get("verdict_non_regress"))
    delta = data.get("full_autonomy_score_delta")
    if isinstance(delta, (int, float)):
        v3_autonomy = float(delta) >= 0.0
    else:
        prev_snap = data.get("full_autonomy_system_ready_previous") or {}
        ps, cs = prev_snap.get("score"), far.get("score") if isinstance(far, dict) else None
        if isinstance(ps, (int, float)) and isinstance(cs, (int, float)):
            v3_autonomy = float(cs) >= float(ps)
        else:
            v3_autonomy = True

    data["v3_acceptance_overlay"] = {
        "kokuzo_four_keys": v3_kok,
        "verdict_non_regress": v3_verdict,
        "autonomy_score_non_regress": v3_autonomy,
        "base_acceptance_pass_from_v1_engine": base_pass,
    }
    final_pass = base_pass and v3_kok and v3_verdict and v3_autonomy
    fr = list(data.get("failure_reasons") or [])
    if not v3_kok:
        fr.append("v3_kokuzo_core_four_keys_missing")
    if not v3_verdict:
        fr.append("v3_overall_verdict_regressed")
    if not v3_autonomy:
        fr.append("v3_full_autonomy_system_ready_score_regressed")
...
def _patch_report_md(md_path: Path, json_path: Path) -> None:
    ...
    v3 = data.get("v3_acceptance_overlay") or {}
    lines = [
        f"# TENMON_FINAL_ACCEPTANCE_PROBE_AND_AUTONOMY_RELOCK_REPORT_V3 (P5)",
        "",
        f"- **generated_at**: `{data.get('generated_at')}`",
        f"- **acceptance_pass**: `{data.get('acceptance_pass')}`",
        f"- **overall_verdict**: `{data.get('overall_verdict')}`",
        f"- **verdict_forward** (strict rank up vs baseline): `{vc.get('verdict_forward')}`",
        f"- **verdict_non_regress**: `{vc.get('verdict_non_regress')}`",
...
def main() -> int:
    ...
    v2_json = auto / "tenmon_final_acceptance_probe_relock_result_v2.json"
    v3_json = auto / OUT_JSON
    ...
    prev_verdict, prev_path = _load_previous_verdict([v3_json, v2_json, v1_json])
        ...
        json.dumps(
            {
                ...
                "v3_acceptance_overlay": final.get("v3_acceptance_overlay"),
    ...
    return 0 if final.get("acceptance_pass") else 2
