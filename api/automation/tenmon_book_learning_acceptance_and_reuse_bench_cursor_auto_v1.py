...
観測専用: binder 上の ARK reuse / discernment / uncertainty 表面をプローブ集合で記録。
"""
...
OUT_JSON = "tenmon_book_learning_acceptance_and_reuse_bench_result_v1.json"
OUT_MD = "tenmon_book_learning_acceptance_and_reuse_bench_report_v1.md"
BEGIN = "<!-- BOOK_LEARNING_BENCH_AUTO_BEGIN -->"
END = "<!-- BOOK_LEARNING_BENCH_AUTO_END -->"
...
def _emit(api_dir: Path) -> tuple[dict | None, str | None]:
    script = api_dir / "scripts" / "tenmon_book_learning_bench_emit_v1.ts"
...
def _patch_md(path: Path, data: dict) -> None:
    ...
    for r in results:
        lines.append(
            f"- `{r.get('probe_id')}` ark=`{r.get('has_ark_reuse')}` mode=`{r.get('discern_source_mode')}` "
            f"sec_risk=`{r.get('katakamuna_secondary_risk')}` tmm_axes=`{r.get('thread_meaning_unresolved_axes')}`",
        )
...
def main() -> int:
    ...
    result = {
        "schema": "TENMON_BOOK_LEARNING_ACCEPTANCE_REUSE_BENCH_RESULT_V1",
        "card": CARD,
        ...
        "nextOnPass": (probe or {}).get("nextOnPass") or "TENMON_BOOK_LEARNING_DEEP_XRAY_AND_QUALITY_FORENSIC_CURSOR_AUTO_V1",
        "nextOnFail": (probe or {}).get("nextOnFail") or "TENMON_BOOK_LEARNING_ACCEPTANCE_AND_REUSE_BENCH_RETRY_CURSOR_AUTO_V1",
