...
# -*- coding: utf-8 -*-
"""
TENMON_KOKUZO_CORE_TENMON_READING_INSERT_CURSOR_AUTO_V2

kokuzo_core に天聞固有読解要約を idempotent 投入する。
DB: $TENMON_DATA_DIR/kokuzo.sqlite（既定 /opt/tenmon-ark-data/kokuzo.sqlite）

summary は V1 カードで確定したユーザー提示文をそのまま使用（既存経典コーパス本文は改変しない）。
差し替えは --summaries-json（{"hokekyo_tenmon_reading": "...", ...} 全4キー必須）
"""
...
CARD = "TENMON_KOKUZO_CORE_TENMON_READING_INSERT_CURSOR_AUTO_V2"
DEFAULT_DATA_DIR = "/opt/tenmon-ark-data"
IMPORTANCE = 5

# V1 と同一の要約（法華・即身成仏・言霊・水火の天聞読み。検索フック用）
...
EXPECTED_KEYS = tuple(DEFAULT_SUMMARIES.keys())
...
def main() -> int:
    ...
    ap.add_argument(
        "--verify-only",
        action="store_true",
        help="4 key の存在のみ確認（書き込みなし・欠けていれば exit 3）",
    )
    ap.add_argument(
        "--summaries-json",
    ...
    if not db_file.is_file():
        print(f"[{CARD}] FAIL: db file missing: {db_file}", file=sys.stderr)
        return 1
    ...
    try:
        ...
        if args.verify_only:
            ok = _verify(conn)
            print(f"[{CARD}] verify-only db={db_file} all_four_present={ok}")
            return 0 if ok else 3

        conn.execute(CREATE_TABLE_SQL)
        cur = conn.cursor()
        for key, summary in summaries.items():
            cur.execute(UPSERT_SQL, (key, summary, float(IMPORTANCE)))
        conn.commit()
        if not _verify(conn):
            print(f"[{CARD}] FAIL: post-upsert verify missing row", file=sys.stderr)
            return 1
    except Exception as e:
