...
観測専用: OCR→settlement→監査→ARK→bench→deep xray→uplift の連鎖 seal。
"""
...
OUT_JSON = "tenmon_book_learning_mainline_seal_result_v1.json"
OUT_MD = "tenmon_book_learning_mainline_seal_report_v1.md"
BEGIN = "<!-- BOOK_LEARNING_MAINLINE_SEAL_AUTO_BEGIN -->"
END = "<!-- BOOK_LEARNING_MAINLINE_SEAL_AUTO_END -->"
...
def _emit(api_dir: Path) -> tuple[dict | None, str | None]:
    script = api_dir / "scripts" / "tenmon_book_learning_mainline_seal_emit_v1.ts"
...
def main() -> int:
    ...
    if probe is not None and not probe_ok:
        fr.extend(probe.get("failure_reasons") or ["seal_probe_acceptance_false"])

    ok = build_ok and probe is not None and probe_ok

    result = {
        "schema": "TENMON_BOOK_LEARNING_MAINLINE_SEAL_RESULT_V1",
        "card": CARD,
        ...
        or "TENMON_ARTIFACT_AND_WORKTREE_HYGIENE_CURSOR_AUTO_V1",
        "nextOnFail": (probe or {}).get("nextOnFail")
        or "TENMON_BOOK_LEARNING_MAINLINE_SEAL_RETRY_CURSOR_AUTO_V1",
