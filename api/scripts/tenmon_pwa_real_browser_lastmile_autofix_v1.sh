...
mkdir -p "$DIR"
ln -sfn "$DIR" /var/log/tenmon/card_TENMON_PWA_REAL_BROWSER_LASTMILE_AUTOFIX_CURSOR_AUTO_V1 2>/dev/null || true
exec > >(tee -a "$DIR/run.log") 2>&1

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
API="$(cd "$SCRIPT_DIR/.." && pwd)"
ROOT="$(cd "$API/.." && pwd)"
# TENMON_PWA_RUNTIME_ENV_RESTORE_V1
...
say "AUTOFIX PY + BROWSER REPROBE"
set +e
PY_ARGS=( "$API/automation/tenmon_pwa_real_browser_lastmile_autofix_v1.py" "$ROOT" "$DIR" --url "$TARGET_URL" )
if [ "$STDOUT_JSON" -eq 1 ]; then
  PY_ARGS+=( --stdout-json )
fi
python3 "${PY_ARGS[@]}" | tee "$DIR/autofix_stdout.json"
PY_EXIT=$?
set -e

cp -f "$API/automation/pwa_real_browser_lastmile_autofix_report.json" "$DIR/" 2>/dev/null || true
cp -f "$API/automation/pwa_real_browser_lastmile_postfix_readiness.json" "$DIR/" 2>/dev/null || true
...
cp -f "$API/automation/generated_cursor_apply/TENMON_PWA_FINAL_AUTOLOOP_COMPLETION_CURSOR_AUTO_V1.md" "$DIR/" 2>/dev/null || true
