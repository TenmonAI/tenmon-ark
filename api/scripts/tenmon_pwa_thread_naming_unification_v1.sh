...
mkdir -p "$DIR"
ln -sfn "$DIR" /var/log/tenmon/card_TENMON_PWA_THREAD_NAMING_UNIFICATION_V1 2>/dev/null || true
...
say "GREP SESSIONID"
rg -n '\bsessionId\b' "$ROOT/web/src" | tee "$DIR/grep_sessionId_websrc.txt" || true
...
say "THREAD NAMING UNIFICATION PY"
set +e
PY_ARGS=( "$API/automation/tenmon_pwa_thread_naming_unification_v1.py" "$ROOT" "$DIR" --gate-json "$DIR/gate_status.json" )
...
cp -f "$API/automation/pwa_thread_naming_unification_trace.json" "$DIR/" 2>/dev/null || true
cp -f "$API/automation/pwa_thread_naming_unification_readiness.json" "$DIR/" 2>/dev/null || true
cp -f "$API/automation/pwa_thread_naming_unification_verdict.json" "$DIR/" 2>/dev/null || true
