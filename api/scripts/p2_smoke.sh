...
echo "$RES" | jq -e '.ok==true and .schemaVersion==1 and .payload.kind=="SEED_PACK_V1"' >/dev/null
echo "$RES" | jq -e '(.payload.items|type)=="array" and (.payload.items|length)>=1' >/dev/null
...
echo "$RES2" | jq -e '.ok==true and .schemaVersion==1' >/dev/null
