...
# tuned: gray + normalize + negate + thr70, psm6
...
# QC json: python code is fixed, text is stdin only (no mixing, no em-dash crash)
...
# DB save: do createdAt in python (never datetime(now) token issues)
