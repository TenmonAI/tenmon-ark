...
mkdir -p "$DIR"
ln -sfn "$DIR" /var/log/tenmon/card_TENMON_PWA_THREAD_URL_CONSTITUTION_V1 2>/dev/null || true
...
say "PWA THREAD URL CONSTITUTION PY"
set +e
PY_ARGS=( "$API/automation/tenmon_pwa_thread_url_constitution_v1.py" "$ROOT" "$DIR" "$BASE" --gate-json "$DIR/gate_status.json" )
...
cp -f "$API/automation/pwa_thread_url_constitution_trace.json" "$DIR/" 2>/dev/null || true
cp -f "$API/automation/pwa_thread_url_constitution_readiness.json" "$DIR/" 2>/dev/null || true
cp -f "$API/automation/pwa_thread_url_constitution_verdict.json" "$DIR/" 2>/dev/null || true
