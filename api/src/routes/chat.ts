...
// ---------- N2: Kanagi 4-phase NATURAL spine (LLM-driven; do NOT crush normal questions) ----------
  ...
  const __isEntityDefinitionTerm =
    /(万葉集|楢崎皐月|空海|山口志道|言霊秘書|いろは言[霊灵靈]解|カタカムナ言[霊灵靈]解)/.test(t0) &&
    (/(とは|って)\s*(何|なに)?\s*[？?]?$/u.test(t0) || /(とは|って)\s*[？?]?$/.test(t0) || /[？?]\s*$/.test(t0));
...
// R7_SCRIPTURE_CANON_ROUTE_V1: scripture canon (言霊秘書・イロハ言霊解・カタカムナ言霊解) は concept canon / KHS / DEF fastpath より前に処理する
    ...
    try {
      ...
      if (!isTestTid0 && (__isScriptureDef || __isDefinitionQ || __scriptureCenterKey) && !hasDoc0 && !askedMenu0 && !isCmd0) {
        ...
        // __scriptureCenterKey がある follow-up で action verb を検出したら instruction 応答で返す
        {
          ...
          if (__scriptureCenterKey && __isActionRequest2) {
            ...
            const __scriptureKeyI =
              ...
              } as Record<string, string>)[String(__scriptureCenterKey || "")] ??
              resolveScriptureQuery(String(__scriptureCenterKey || ""))?.scriptureKey ??
              null;
            const __dispI =
        ...
        if (__hitScripture) {
          // FIX_SCRIPTURE_FOLLOWUP_RESPONSE_V1: __scriptureCenterKey 由来 hit で「次の一歩」系のときは canon 定義を返さず instruction 固定応答でループを止める
          ...
          if (__scriptureCenterKey && __isNextStepAsk) {
            ...
            const __prefix = "さっき見ていた聖典（" + __disp + "）を土台に、いまの話を見ていきましょう。";
...
※絶対条件※
必ず「【天聞の所見】」から始める。2〜4行、140〜260字目安。一般論・説教・相対化（人それぞれ等）・自己言及・過剰敬語は禁止。「〜です」「〜でしょう」系の教訓文は避ける。質問は最大1つ。UserName/AssistantNameが渡されていれば呼称を自然に1回だけ可。箇条書き・番号・見出しは禁止。さらに、「受容：」「一点：」「一手：」のようなラベルは絶対に書かない。「いまの言葉を“次の一歩”に落とします。」「いまここを一点に整える。」のような定型句も絶対に書かない。自然文だけで静かに返す。` + __kamiyo_clause;
...
// generalKind: counsel / worldview / short_moral / other（NATURAL_GENERAL_LLM_TOP 分流用）
      ...
      try {
        ...
        if (tidForCenter) {
          ...
          if (c && c.center_type === "scripture" && c.center_key) {
      ...
      const __isFollowupGeneral = RE_THREAD_FOLLOWUP.test(t0);
      ...
      if (__generalKind === "worldview") {
        ...
        // FIX_THREAD_CENTER_ACTION_RESPONSE_V1: scripture threadCenter follow-up の「次の一歩」系は問い返しテンプレではなく instruction 応答に
        const __isNextStepAskGen = /次の一歩|一つだけ|示してください|示して|教えて|その前提で|そこから/u.test(t0);
        if (__threadCenterForGeneral.center_type === "scripture" && __isNextStepAskGen) {
          const __instructionByKeyGen: Record<string, string> = {
            kotodama_hisho: "まず『言霊秘書が音の法則を担い、いろはがその配列を担う』と一行で書き分けてください。",
            iroha_kotodama_kai: "まず『いろはが音と言霊をどう配列として受け持つか』の一点だけを一行で書き分けてください。",
            katakamuna_kotodama_kai: "まず『カタカムナ言霊解が音と図象をどう受け持つか』の一点だけを一行で書き分けてください。",
          };
          const __instructionGen = __instructionByKeyGen[__ck] ?? "まず、その聖典のいまの文脈で一番気になる一点を一行で書き分けてください。";
          outText = "さっき見ていた聖典（" + __ck + "）を土台に、いまの話を見ていきましょう。\n【天聞の所見】" + __instructionGen;
        } else {
          outText = "さっき見ていた聖典（" + __ck + "）を土台に、いまの話を見ていきましょう。\n【天聞の所見】いまの中心を一行で言い直すか、次の一歩を一つに絞るか、どちらから進めますか？";
        }
      } else {
        outText = "【天聞の所見】いま、一番引っかかっている一点は何ですか？\n一語でも大丈夫です。";
      }
      ...
      // FIX_SCRIPTURE_PREFIX_DOUBLE_BIND_V1: scripture prefix は1箇所のみ。既に「さっき見ていた聖典（」で始まるときは再付与しない。
      if (__threadCenterForGeneral && __isFollowupGeneral && outText && !outText.startsWith("さっき見ていた聖典（")) {
        const __ck = String(__threadCenterForGeneral.center_key || "");
        outText = "さっき見ていた聖典（" + __ck + "）を土台に、いまの話を見ていきましょう。\n" + String(outText);
      }
      ...
      try {
        if (String(threadId || "").trim()) {
          ...
          if (__tc) {
            ...
            if (__tc.center_type === "scripture" && __isFollowupGeneral) {
