...
・それ以外の一般語は150〜500文字で簡潔に`;
      ...
      try {
        ...
        const llmRes = await llmChat({ system: DEF_SYSTEM, user: t0, history: memoryReadSession(String(threadId || ""), 8), maxTokens: __isDeepDomainDef ? 3000 : 2000, timeout: __isDeepDomainDef ? 45000 : 30000 });
        if (llmRes?.ok === false || !String(llmRes?.text ?? "").trim()) {
...
let outText = "";
      ...
      if (__hasSukuyouOracle) {
        ...
        try {
          const __genSystemWithEvidence = GEN_SYSTEM + __sukuyouContextClause;
...
let outText = "";
      ...
      try {
        const llmRes: any = await llmChat({ system: KANAGI_SYSTEM_PROMPT, user: t0, history: memoryReadSession(String(threadId || ""), 8), maxTokens: 2000, timeout: 20000 });
        if (llmRes?.ok === false || !String(llmRes?.text ?? "").trim()) {
