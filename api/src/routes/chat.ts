...
// generalKind: counsel / worldview / short_moral / other（NATURAL_GENERAL_LLM_TOP 分流用）
      ...
      console.log("[GEN_GENERAL_CLAMP_AFTER]", { out: String(outText ?? "").slice(0, 240) });

      // generalKind 分流: worldview は counsel テンプレを使わない。short_moral は typo を避け 2〜3 行固定。
      if (__generalKind === "worldview") {
        outText = "【天聞の所見】その問いは、世界の見え方に触れている。\n原因を一つに絞らず、いま自分に引っかかっている一点だけ教えてください。";
      } else if (__generalKind === "short_moral") {
        outText = "【天聞の所見】「恨まない」「許す」は、全部を手放すことではない。\n自分を守る線を引きつつ、その外側に少しだけ余白を残す。いま、線を引きたいのはどこですか？";
      }

      // threadCenter(scripture) かつ follow-up のとき、冒頭 1 行だけ scripture continuity の prefix を入れる。
...
const __heartNorm = normalizeHeartShape(__heart);
      ...
      // 先頭欠損防止: responseComposer 内の貪欲 replace が効かないよう、「いまの言葉を…」のみ事前に安全除去。scripture prefix / worldview / short_moral は触らない。
      const __safeForComposer = __cleanLlmFrame(String(outText ?? ""));
      const __composed = responseComposer({
        response: __safeForComposer,
        rawMessage: String(message ?? ""),
        mode: "NATURAL",
        routeReason: "NATURAL_GENERAL_LLM_TOP",
        truthWeight: Number(__truthWeight ?? 0),
