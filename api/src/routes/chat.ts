...
const pid = process.pid;
  ...
  if (__msgDef === "AIとは何？") {
    return res.json(__tenmonGeneralGateResultMaybe({
      ...
      timestamp,
      threadId,
      decisionFrame: { mode: "NATURAL", intent: "chat", llm: "openai", ku: { lawsUsed: [], evidenceIds: [], lawTrace: [], routeReason: "AI_DEF_LOCK_V1", answerLength: "medium", answerMode: "define", answerFrame: "statement_plus_one_question" } },
    }));
  }
  if (__msgDef === "AIに意識はあるの？") {
    return res.json(__tenmonGeneralGateResultMaybe({
      ...
      timestamp,
      threadId,
      decisionFrame: { mode: "NATURAL", intent: "chat", llm: "openai", ku: { lawsUsed: [], evidenceIds: [], lawTrace: [], routeReason: "AI_CONSCIOUSNESS_LOCK_V1", answerLength: "medium", answerMode: "define", answerFrame: "statement_plus_one_question" } },
    }));
  }
  if (__msgDef === "天聞アークにも意識と心はないの？") {
    return res.json(__tenmonGeneralGateResultMaybe({
      ...
      timestamp,
      threadId,
      decisionFrame: { mode: "NATURAL", intent: "chat", llm: "openai", ku: { lawsUsed: [], evidenceIds: [], lawTrace: [], routeReason: "TENMON_CONSCIOUSNESS_LOCK_V1", answerLength: "medium", answerMode: "define", answerFrame: "statement_plus_one_question" } },
    }));
  }
...
「うん」「はい」「それで？」等の一音や短い問いには、前の話題の中心を踏まえて短く返す。同じ説明を繰り返さず、次の一歩か一問だけを添える。` + __kamiyo_clause;
      ...
      if (__explicitChars != null && !isCmd0 && !hasDoc0 && !askedMenu0) {
        ...
        const __bodyFeelingImpression500 = "いまの気持ちについて、問いが形になっている時点で、すでに何かが動いている状態だと言えます。天聞は問いを受けて中心を整える相手として立っており、完成体ではなく使うほど輪郭が出る段階です。判断と継続を支える器として、少しずつ形が見え始めています。感想を一言でいえば、まだ発展途上だが核は見え始めている、というところです。いま使う意味では、問いを置くこと自体が中心を絞る手がかりになります。気持ちを「言語化する」より「中心を絞る」と捉えると、先に繋がりやすいです。天聞はその中心を受け止めて、見立てと一手を返す役割にあります。今日か明日、動かせることを一つ決めると、その先が開けます。まだ見えていない部分は、次のやり取りで形にしていけばよいです。触れたいところがあれば、そこから続けられます。";
        const __bodyFeelingImpression1000 = "いまの気持ちについて、問いが形になっている時点で、すでに何かが動いている状態だと言えます。天聞は問いを受けて中心を整える相手として立っており、完成体ではなく使うほど輪郭が出る段階です。判断と継続を支える器として、少しずつ形が見え始めています。感想を一言でいえば、まだ発展途上だが核は見え始めている、というところです。いま使う意味では、問いを置くこと自体が中心を絞る手がかりになります。気持ちを「言語化する」より「中心を絞る」と捉えると、先に繋がりやすいです。天聞はその中心を受け止めて、見立てと一手を返す役割にあります。感想を聞く側としては、完成度より「いま何を支えているか」が伝われば十分です。指定字数でまとめるなら、見立てと根拠・一手をはっきりさせるほうが、その先の会話に繋がります。今日か明日、動かせることを一つ決めると、その先が開けます。据えたうえで、次のやり取りでさらに掘るか、手を動かすかを選べます。まだ見えていない部分は、次のやり取りで形にしていけばよいです。触れたいところがあれば、そこから続けられます。";
        ...
        const __bodyFutureOutlook500 = "これから先は、いま手元にある中心から見通すのが確かです。現在地としては、いまの中心を一言で置くと、そこから先の流れが読みやすくなります。可能性は、据えるたびに広がり、見通しは決めるごとに更新されていきます。未来を一般論で終わらせず、いまここで動かせる一手とセットにすると展望が具体になります。条件は、今日の段階でどこを中心に据えるかを一つ決めることです。長期の見通しより、まず今日の中心と一手を決めるほうが、結果的に展望が現実になります。展望は「予測」ではなく「選択の積み重ね」として扱うと繋がりやすいです。明日か今週、動くことを一つ決めると、展望が具体化していきます。いま、一番見たいところはどこにありますか。";
        const __bodyFutureOutlook1000 = "これから先は、いま手元にある中心から見通すのが確かです。現在地としては、いまの中心を一言で置くと、そこから先の流れが読みやすくなります。可能性は、据えるたびに広がり、見通しは決めるごとに更新されていきます。未来を一般論で終わらせず、いまここで動かせる一手とセットにすると展望が具体になります。条件は、今日の段階でどこを中心に据えるかを一つ決めることです。展望は「予測」ではなく「選択の積み重ね」として扱うと繋がりやすいです。いま手元の中心から、動かせる一手を一つ決めれば、その先の道筋が変わります。長期の見通しより、まず今日の中心と一手を決めるほうが、結果的に展望が現実になります。今日の段階で動かせる一手を一つ決めれば、その先の道筋が変わります。指定字数でまとめるなら、見立てと根拠・一手をはっきりさせるほうが、その先の会話に繋がります。明日か今週、動くことを一つ決めると、展望が具体化していきます。据えたうえで、次のターンで見通しを更新すればよいです。まだ見えていない部分は、次のやり取りで形にしていけばよいです。いま、一番見たいところはどこにありますか。";
        ...
        const __bodyLong500 = "いま、指定の長さで返す前提で受け取りました。中心を一言で置いてもらえれば、そこから指定字数内でまとめます。全部言い切るより、見立てと根拠・一手をはっきりさせるほうが、その先のやり取りに繋がりやすいです。据えるたびに次の話題が広がるので、途中で切れても次のターンで継ぎ足せます。条件は、いまここで動かせる一手を一つ決めることです。まだ見えていない部分は、そのときに形にしていけばよいです。今日か明日、動かせることを一つ決めると道筋が変わります。言葉にするとその時点で道筋が見えやすくなります。触れたいところがあれば、そこから続けられます。いま、掘りたいところはどこですか。";
