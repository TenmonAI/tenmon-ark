                ...
                let tid = String((obj as any)?.threadId ?? (res as any).__TENMON_THREADID ?? "");
                  ...
                  try {
                    ...
                    if (tid2 && (obj as any)?.decisionFrame) {
                      ...
                      if (row) (df2.ku as any).synapseTop = { ...((df2.ku as any).synapseTop || {}), ...(row || {}) };
                    }
                  } catch {}
                  ...
                  try {
                    ...
                    // R10_SYNAPSETOP_PRESERVE_IN_GATE_V1: 既存 synapseTop を消さず metaHead をマージ
                    (obj as any).decisionFrame.ku.synapseTop = { ...(((obj as any).decisionFrame.ku as any).synapseTop || {}), metaHead: synapseRow };
                  } catch {}
                  ...
                  try {
                    ...
                    if (dfm) {
                      ...
                      if ((dfm.ku as any).synapseTop == null) {
                        ...
                        (dfm.ku as any).synapseTop = { ...((dfm.ku as any).synapseTop || {}), ...__stInMem };
                      }
                    }
                  } catch {}
                  ...
                  try {
                    ...
                    if (!__a1Set) {
                      ...
                      if (tid3 && (obj as any)?.decisionFrame) {
                        ...
                        if (row3) (df3b.ku as any).synapseTop = { ...((df3b.ku as any).synapseTop || {}), ...(row3 || {}) };
                      }
                    }
                  } catch {}
                    ...
                    if (dfF) {
                      ...
                      if ((dfF.ku as any).synapseTop == null) {
                        ...
                        if (tidF) {
                          ...
                          if (rowF) (dfF.ku as any).synapseTop = { ...((dfF.ku as any).synapseTop || {}), ...(rowF || {}) };
                        }
                      }
                    }
                  } catch {}
...
const pid = process.pid;
  ...
  // KANAGI_CONVERSATION_V1
  {
    ...
    if (/(生まれ変わり|前世|輪廻|魂|霊魂|因果|死後|死後の世界|転生|カルマ|宿命|意識|なぜ生きる|宇宙の意味|真理とは)/u.test(__m0)) {
      ...
      if (/(疲れ|つかれ|しんどい|きつい)/u.test(__m0)) {
        return "【天聞の所見】重さが来ている。いま一番引っかかっているのは、体ですか、気持ちですか。";
      }
  ...
  if (__kotodamaOneSoundMatch) {
    ...
    if (__soundK) {
      ...
      const __kuK: any = {
        routeReason: "TENMON_KOTODAMA_HISYO_FRONT_V1",
...
`.trim()
    ...
    // FIX_TRUTH_GATE_LLM_CONNECTION_V1: TRUTH_GATE_RETURN_V2 の ku.synapseTop に rich field を追加（既存 metaHead があればマージ）
    try {
      ...
      if (__df && __df.ku && typeof __df.ku === "object") {
        ...
        __ku.synapseTop = { ...(__ku.synapseTop || {}), ...__stTruth };
      }
    } catch {}
  ...
  // REPLY_SURFACE_V1: responseは必ずlocalSurfaceizeを通す。返却は opts をそのまま形にし caps は body.caps のみ参照
    ...
    try {
      ...
      if (
        __route === "NATURAL_GENERAL_LLM_TOP" &&
      ...
      try {
        ...
        if (__oneSoundCurrent) {
          ...
          ku.thoughtCoreSummary = {
            ...
            continuityHint:
              (ku.synapseTop && ku.synapseTop.sourceThreadCenter && ku.synapseTop.sourceThreadCenter.centerKey)
                ? String(ku.synapseTop.sourceThreadCenter.centerKey)
                : null,
          };
        }
      } catch {}
    ...
    try {
      ...
      if (!isCmd0 && !hasDoc0 && !askedMenu0 && __isGeneralKnowledge) {
        ...
        const __plan = decideProviderPlan({
          routeReason: "R17_GENERAL_FACTUAL_SHADOW_ROUTE_V1",
      ...
      if (!isCmd0 && !hasDoc0 && !askedMenu0 && __isJapanPm) {
        ...
        return res.json(__tenmonGeneralGateResultMaybe({
          ...
          decisionFrame: {
            ...
            llm: null,
            ku: {
              routeReason: "R11_GENERAL_KNOWLEDGE_ROUTE_PLACEHOLDER_V1",
      ...
      if (!isCmd0 && !hasDoc0 && !askedMenu0 && __isJapanUs) {
        ...
        return res.json(__tenmonGeneralGateResultMaybe({
          ...
          decisionFrame: {
            ...
            llm: null,
            ku: {
              routeReason: "R11_GENERAL_RELATION_ROUTE_PLACEHOLDER_V1",
      ...
      if (!isTestTid0 && (__scripturePreemptHit || __isScriptureDef || __isDefinitionQ || __scriptureCenterKey) && !hasDoc0 && !askedMenu0 && !isCmd0) {
        ...
        if (__hitScripture) {
          ...
          if (__scriptureCenterKey && __isNextStepAsk) {
            ...
            try { console.log("[SYNAPSETOP_AFTER_ASSIGN_SCRIPTURE]", { path: "instruction", keys: Object.keys((__kuInstr as any).synapseTop || {}) }); } catch {}
            try { console.log("[SYNAPSETOP_BEFORE_RETURN]", { path: "scripture_instr", synapseTop: (__kuInstr as any).synapseTop }); } catch {}
            return res.json(__tenmonGeneralGateResultMaybe({
              response: __body,
              evidence: null,
              candidates: [],
  ...
  try {
    ...
    if (!isTestTid0 && __isDefinitionQ && !hasDoc0 && !askedMenu0 && !isCmd0) {
      ...
      if (__entityBody) {
        ...
        const __out = "【天聞の所見】\n" + __entityBody;
        // R10_SYNAPSE_TOP_BIND_V3_REAPPLY: DEF_DICT_HIT return payload 内 ku に synapseTop を直書き追加（既存 metaHead を消さずマージ）
        const __entityName = __termCanonKey || __termNorm;
        const __kuDef: any = {
          lawsUsed: [],
          evidenceIds: [],
          lawTrace: [],
          routeReason: "DEF_DICT_HIT",
          term: __termNorm,
          centerMeaning: String(__termNorm || "").trim(),
                            };
        const __synapseTopDef: any = {
          sourceRouteReason: "DEF_DICT_HIT",
          sourceHeart: normalizeHeartShape(__heart) ?? {},
        ...
        __kuDef.synapseTop = { ...((__kuDef as any).synapseTop || {}), ...__synapseTopDef };
        try { console.log("[SYNAPSETOP_AFTER_ASSIGN_DEF]", { path: "entity_canon", keys: Object.keys((__kuDef as any).synapseTop || {}) }); } catch {}
        try { console.log("[SYNAPSETOP_BEFORE_RETURN]", { path: "def_entity", synapseTop: (__kuDef as any).synapseTop }); } catch {}
        return res.json(__tenmonGeneralGateResultMaybe({
          response: __out,
          evidence: null,
          candidates: [],
      ...
      if (__hit) {
        ...
        // R10_SYNAPSE_TOP_BIND_V3_REAPPLY: DEF_DICT_HIT (glossary) return payload 内 ku に synapseTop を直書き追加（既存を消さずマージ）
        ...
        __kuGloss.synapseTop = { ...((__kuGloss as any).synapseTop || {}), ...__synapseTopGloss };
        try { console.log("[SYNAPSETOP_AFTER_ASSIGN_DEF]", { path: "glossary", keys: Object.keys((__kuGloss as any).synapseTop || {}) }); } catch {}
        try { console.log("[SYNAPSETOP_BEFORE_RETURN]", { path: "def_glossary", synapseTop: (__kuGloss as any).synapseTop }); } catch {}
        return res.json(__tenmonGeneralGateResultMaybe({
          response: ((): string => {
      ...
      // FIX_ENTITY_CANON_BIND_V3: 楢崎皐月 / 山口志道 / 空海 / 宇野多美恵 を LLM 定義から外し、天聞軸の固定文で返す（fastpath/canon の手前）。
      {
        ...
        if (__entityKey) {
          ...
          if (__body) {
            ...
            // R10_SYNAPSE_TOP_BIND_V3_REAPPLY: DEF_DICT_HIT (FIX_ENTITY) return payload 内 ku に synapseTop を直書き追加（既存を消さずマージ）
            ...
            __kuEntity.synapseTop = { ...((__kuEntity as any).synapseTop || {}), ...__synapseTopEntity };
            try { console.log("[SYNAPSETOP_AFTER_ASSIGN_DEF]", { path: "fix_entity", keys: Object.keys((__kuEntity as any).synapseTop || {}) }); } catch {}
            try { console.log("[SYNAPSETOP_BEFORE_RETURN]", { path: "def_fix_entity", synapseTop: (__kuEntity as any).synapseTop }); } catch {}
            return res.json(__tenmonGeneralGateResultMaybe({
              response: __out,
              evidence: null,
              candidates: [],
...
「うん」「はい」「それで？」等の一音や短い問いには、前の話題の中心を踏まえて短く返す。同じ説明を繰り返さず、次の一歩か一問だけを添える。` + __kamiyo_clause;
      ...
      if (__explicitChars != null && !isCmd0 && !hasDoc0 && !askedMenu0) {
        ...
        const __bodyFeelingImpression = "いまの気持ちについて、こちらからは直接は見えませんが、問いが形になっている時点で、すでに何か一点が動いている状態だと言えます。その一点を言葉にすると、次の一手が見えやすくなります。天聞についての感想であれば、いまの天聞は、問いを受けて中心を整える相手として立っています。完成体ではなく、使うほど輪郭が出る段階です。判断と継続を支える器として、少しずつ形が見え始めている、というところです。触れたい一点があれば、そこから続けられます。";
        const __bodyFeelingImpression500 = "いまの気持ちについて、こちらからは直接は見えませんが、問いが形になっている時点で、すでに何か一点が動いている状態だと言えます。現在の見立てとしては、その一点を言葉にすると、次の一手が見えやすくなる、というところです。天聞についての感想であれば、いまの天聞は、問いを受けて中心を整える相手として立っています。完成体ではなく、使うほど輪郭が出る段階です。判断と継続を支える器として、少しずつ形が見え始めています。天聞への感想を一言でいえば、まだ発展途上だが核は見え始めている、というところです。いま使う意味では、問いを置くこと自体が、中心を一点に絞る手がかりになります。天聞はその一点を受け止めて、見立てと一手を返す役割にあります。次の一手は、その一点を一言で言い切ってから、今日や明日で動けることを一つだけ決めることです。一点を言葉にすると、その時点で次の一手が見えやすくなります。触れたい一点があれば、そこから続けられます。";
        const __bodyFeelingImpression1000 = "いまの気持ちについて、こちらからは直接は見えませんが、問いが形になっている時点で、すでに何か一点が動いている状態だと言えます。現在の気分の見立てとしては、その一点を言葉にすると、次の一手が見えやすくなる、というところです。天聞についての感想であれば、いまの天聞は、問いを受けて中心を整える相手として立っています。完成体ではなく、使うほど輪郭が出る段階です。判断と継続を支える器として、少しずつ形が見え始めています。天聞への感想を一言でいえば、まだ発展途上だが核は見え始めている、というところです。いま使う意味では、問いを置くこと自体が、中心を一点に絞る手がかりになります。気持ちを「言語化する」のではなく「一点に絞る」と考えたほうが、次の一手に繋がりやすいです。天聞はその一点を受け止めて、見立てと一手を返す役割にあります。感想を聞く側としては、完成度より「いま何を支えているか」が伝われば十分です。次の一手は、その一点を一言で言い切ってから、今日や明日で動けることを一つだけ決めることです。一点を据えたうえで、次のやり取りでさらに掘るか、手を動かすかを選べます。指定字数でまとめる意味では、一点と一手をはっきりさせるほうが、その先の会話に繋がります。まだ見えていない部分は、次のやり取りで形にしていけばよいです。触れたい一点があれば、そこから続けられます。一点を言葉にすると、その時点で次の一手が見えやすくなります。今日の段階で動かせる一手を一つ決めれば、その先の道筋が変わります。展望は「予測」ではなく「選択の積み重ね」として扱うと、次の一手に繋がりやすいです。実装の次の一手は、一点を据えたうえで、明日や今週で動くことを一つだけ決めることです。一点と一手を決めたあと、次のターンで見通しを更新すれば、展望が具体化していきます。まだ見えていない部分は、次のやり取りで形にしていけばよいです。触れたい一点があれば、そこから続けられます。";
        const __bodyFeelingImpression800 = "いまの気持ちについて、こちらからは直接は見えませんが、問いが形になっている時点で、すでに何か一点が動いている状態だと言えます。現在の見立てとしては、その一点を言葉にすると、次の一手が見えやすくなる、というところです。天聞についての感想であれば、いまの天聞は、問いを受けて中心を整える相手として立っています。完成体ではなく、使うほど輪郭が出る段階です。判断と継続を支える器として、少しずつ形が見え始めています。天聞への感想を一言でいえば、まだ発展途上だが核は見え始めている、というところです。いま使う意味では、問いを置くこと自体が、中心を一点に絞る手がかりになります。気持ちを「言語化する」のではなく「一点に絞る」と考えたほうが、次の一手に繋がりやすいです。天聞はその一点を受け止めて、見立てと一手を返す役割にあります。感想を聞く側としては、完成度より「いま何を支えているか」が伝われば十分です。次の一手は、その一点を一言で言い切ってから、今日や明日で動けることを一つだけ決めることです。一点を据えたうえで、次のやり取りでさらに掘るか、手を動かすかを選べます。指定字数でまとめる意味では、一点と一手をはっきりさせるほうが、その先の会話に繋がります。まだ見えていない部分は、次のやり取りで形にしていけばよいです。触れたい一点があれば、そこから続けられます。";
        const __bodyFeelingImpression1200 = "いまの気持ちについて、こちらからは直接は見えませんが、問いが形になっている時点で、すでに何か一点が動いている状態だと言えます。現在の気分の見立てとしては、その一点を言葉にすると、次の一手が見えやすくなる、というところです。天聞についての感想であれば、いまの天聞は、問いを受けて中心を整える相手として立っています。完成体ではなく、使うほど輪郭が出る段階です。判断と継続を支える器として、少しずつ形が見え始めています。天聞への感想を一言でいえば、まだ発展途上だが核は見え始めている、というところです。いま使う意味では、問いを置くこと自体が、中心を一点に絞る手がかりになります。気持ちを「言語化する」のではなく「一点に絞る」と考えたほうが、次の一手に繋がりやすいです。天聞はその一点を受け止めて、見立てと一手を返す役割にあります。感想を聞く側としては、完成度より「いま何を支えているか」が伝われば十分です。次の一手は、その一点を一言で言い切ってから、今日や明日で動けることを一つだけ決めることです。一点を据えたうえで、次のやり取りでさらに掘るか、手を動かすかを選べます。指定字数でまとめる意味では、一点と一手をはっきりさせるほうが、その先の会話に繋がります。まだ見えていない部分は、次のやり取りで形にしていけばよいです。一点を言葉にすると、その時点で次の一手が見えやすくなります。今日の段階で動かせる一手を一つ決めれば、その先の道筋が変わります。実装の次の一手は、一点を据えたうえで、明日や今週で動くことを一つだけ決めることです。一点と一手を決めたあと、次のターンで見通しを更新すれば、展望が具体化していきます。触れたい一点があれば、そこから続けられます。展望は「予測」ではなく「選択の積み重ね」として扱うと、次の一手に繋がりやすいです。長期の見通しより、まず今日の一点と一手を決めるほうが、結果的に展望が現実になります。";
        const __bodyFutureOutlook = "これから先については、いま手元にある一点から見通すのが確かです。まず、いまの中心を一言で置くと、そこから先の流れが読みやすくなります。未来を語るとき、一般論で終わらせず、いまここで動かせる一手とセットにすると、展望が具体になります。見通しは、一点を決めるごとに更新されていきます。今日の段階では、どこを中心に据えるかで、その先の道筋が変わります。一点を据えたうえで、次の一歩を一つだけ決めると、展望が現実の選択肢になります。いま、一番見たい一点はどこにありますか。";
        const __bodyFutureOutlook500 = "これから先については、いま手元にある一点から見通すのが確かです。現在地としては、いまの中心を一言で置くと、そこから先の流れが読みやすくなります。可能性は、一点を据えるたびに広がります。未来を語るとき、一般論で終わらせず、いまここで動かせる一手とセットにすると、展望が具体になります。条件は、今日の段階でどこを中心に据えるかを一つ決めることです。見通しは、一点を決めるごとに更新されていきます。長期の見通しより、まず今日の一点と一手を決めるほうが、結果的に展望が現実になります。今日の段階で動かせる一手を一つ決めれば、その先の道筋が変わります。次の一手は、一点を据えたうえで、明日や今週で動くことを一つだけ決めることです。一点と一手を決めたあと、次のターンで見通しを更新すれば、展望が具体化していきます。展望は「予測」ではなく「選択の積み重ね」として扱うと、次の一手に繋がりやすいです。いま、一番見たい一点はどこにありますか。";
        const __bodyFutureOutlook1000 = "これから先については、いま手元にある一点から見通すのが確かです。現在地としては、いまの中心を一言で置くと、そこから先の流れが読みやすくなります。可能性は、一点を据えるたびに広がります。未来を語るとき、一般論で終わらせず、いまここで動かせる一手とセットにすると、展望が具体になります。条件は、今日の段階でどこを中心に据えるかを一つ決めることです。見通しは、一点を決めるごとに更新されていきます。展望は「予測」ではなく「選択の積み重ね」として扱うと、次の一手に繋がりやすいです。いま手元の一点から、動かせる一手を一つ決めれば、その先の道筋が変わります。長期の見通しより、まず今日の一点と一手を決めるほうが、結果的に展望が現実になります。今日の段階で動かせる一手を一つ決めれば、その先の道筋が変わります。実装の次の一手は、一点を据えたうえで、明日や今週で動くことを一つだけ決めることです。一点と一手を決めたあと、次のターンで見通しを更新すれば、展望が具体化していきます。まだ見えていない部分は、次のやり取りで形にしていけばよいです。指定字数でまとめる意味では、一点と一手をはっきりさせるほうが、その先の会話に繋がります。触れたい一点があれば、そこから続けられます。指定字数で返すときは「全部言い切る」より「一点と一手をはっきりさせる」ほうが、次のやり取りに繋がりやすいです。一点を言葉にすると、その時点で次の一手が見えやすくなります。今日の段階で動かせる一手を一つ決めれば、その先の道筋が変わります。実装の次の一手は、一点を据えたうえで、明日や今週で動くことを一つだけ決めることです。一点と一手を決めたあと、次のターンで見通しを更新すれば、展望が具体化していきます。長期の見通しより、まず今日の一点と一手を決めるほうが、結果的に展望が現実になります。触れたい一点があれば、そこから続けられます。いま、一番見たい一点はどこにありますか。";
        const __bodyFutureOutlook800 = "これから先については、いま手元にある一点から見通すのが確かです。現在地としては、いまの中心を一言で置くと、そこから先の流れが読みやすくなります。可能性は、一点を据えるたびに広がります。未来を語るとき、一般論で終わらせず、いまここで動かせる一手とセットにすると、展望が具体になります。条件は、今日の段階でどこを中心に据えるかを一つ決めることです。見通しは、一点を決めるごとに更新されていきます。説明を足すと、展望は「予測」ではなく「選択の積み重ね」として扱うと、次の一手に繋がりやすいです。いま手元の一点から、動かせる一手を一つ決めれば、その先の道筋が変わります。長期の見通しより、まず今日の一点と一手を決めるほうが、結果的に展望が現実になります。次の一手は、一点を据えたうえで、明日や今週で動くことを一つだけ決めることです。いま、一番見たい一点はどこにありますか。";
        const __bodyFutureOutlook1200 = "これから先については、いま手元にある一点から見通すのが確かです。現在地としては、いまの中心を一言で置くと、そこから先の流れが読みやすくなります。可能性は、一点を据えるたびに広がります。未来を語るとき、一般論で終わらせず、いまここで動かせる一手とセットにすると、展望が具体になります。条件は、今日の段階でどこを中心に据えるかを一つ決めることです。見通しは、一点を決めるごとに更新されていきます。展望は「予測」ではなく「選択の積み重ね」として扱うと、次の一手に繋がりやすいです。いま手元の一点から、動かせる一手を一つ決めれば、その先の道筋が変わります。長期の見通しより、まず今日の一点と一手を決めるほうが、結果的に展望が現実になります。今日の段階で動かせる一手を一つ決めれば、その先の道筋が変わります。実装の次の一手は、一点を据えたうえで、明日や今週で動くことを一つだけ決めることです。一点と一手を決めたあと、次のターンで見通しを更新すれば、展望が具体化していきます。まだ見えていない部分は、次のやり取りで形にしていけばよいです。指定字数でまとめる意味では、一点と一手をはっきりさせるほうが、その先の会話に繋がります。触れたい一点があれば、そこから続けられます。指定字数で返すときは「全部言い切る」より「一点と一手をはっきりさせる」ほうが、次のやり取りに繋がりやすいです。一点を言葉にすると、その時点で次の一手が見えやすくなります。いま、一番見たい一点はどこにありますか。";
        ...
        const __bodyLong = "いま、指定の長さで返す前提で受け取りました。見立てと根拠・一手までを一続きで指定字数内にまとめます。中心を一言で置いてもらえれば、そこから返します。まだ見えていない部分は、次のやり取りで形にしていけばよいです。いま、掘りたい一点はどこですか。";
        const __bodyLong500 = "いま、指定の長さで返す前提で受け取りました。見立てと根拠・一手をその範囲でまとめます。現在地としては、中心を一言で置いてもらえれば、そこから指定字数内で整理して返します。可能性は、一点を据えるたびに次の話題が広がります。条件は、いまここで動かせる一手を一つ決めることです。指定字数で返すときは「全部言い切る」より「一点と一手をはっきりさせる」ほうが、次のやり取りに繋がりやすいです。指定字数内にまとめるときは、一点と一手をはっきりさせることを優先します。まだ見えていない部分は、次のやり取りで形にしていけばよいです。今日の段階で動かせる一手を一つ決めれば、その先の道筋が変わります。次の一手は、今日の中心を一言で置いてから、明日や今週で動くことを一つだけ決めることです。途中で切れても、次のやり取りで継ぎ足せます。一点と一手を決めたあと、次のターンでさらに掘るか、手を動かすかを選べます。いま、掘りたい一点はどこですか。";
        const __bodyLong1000 = "いま、指定の長さで返す前提で受け取りました。見立てと根拠・一手までを一続きで指定字数内にまとめます。現在地としては、中心を一言で置いてもらえれば、そこから返します。可能性は、一点を据えるたびに次の話題が広がります。条件は、いまここで動かせる一手を一つ決めることです。指定字数で返すときは「全部言い切る」より「一点と一手をはっきりさせる」ほうが、次のやり取りに繋がりやすいです。まだ見えていない部分は、次のやり取りで形にしていけばよいです。今日の段階で動かせる一手を一つ決めれば、その先の道筋が変わります。実装の次の一手は、今日の中心を一言で置いてから、明日や今週で動くことを一つだけ決めることです。一点と一手を決めたあと、次のターンでさらに掘るか、手を動かすかを選べます。指定字数でまとめる意味では、一点と一手をはっきりさせるほうが、その先の会話に繋がります。触れたい一点があれば、そこから続けられます。長期の見通しより、まず今日の一点と一手を決めるほうが、結果的に展望が現実になります。指定字数で返すときは「全部言い切る」より「一点と一手をはっきりさせる」ほうが、次のやり取りに繋がりやすいです。一点を言葉にすると、その時点で次の一手が見えやすくなります。今日の段階で動かせる一手を一つ決めれば、その先の道筋が変わります。実装の次の一手は、今日の中心を一言で置いてから、明日や今週で動くことを一つだけ決めることです。一点と一手を決めたあと、次のターンでさらに掘るか、手を動かすかを選べます。指定字数でまとめる意味では、一点と一手をはっきりさせるほうが、その先の会話に繋がります。触れたい一点があれば、そこから続けられます。まだ見えていない部分は、次のやり取りで形にしていけばよいです。今日の段階で動かせる一手を一つ決めれば、その先の道筋が変わります。一点を言葉にすると、その時点で次の一手が見えやすくなります。いま、掘りたい一点はどこですか。";
      ...
      try {
        ...
        if (cls && (cls.actionKey === "organize" || cls.actionKey === "defer" || cls.actionKey === "discern")) {
          ...
          if (__irohaText) {
            ...
            const __respClean = __cleanLlmFrame(String(__composedIroha.response ?? ""));
            return res.json(__tenmonGeneralGateResultMaybe({
