...
function detectExplicitLongCharsV1(input: string): number {
  const m = String(input || "").match(/([1-9]\d{2,4})\s*字/);
  if (!m) return 0;
...
function buildTenmonLongBodyMapV1(args: {
  userMessage: string;
  base: string;
  minimumFloor: number;
}): string {
  const userMessage = String(args.userMessage || "").trim();
  let out = String(args.base || "").trim();
  const minimumFloor = Math.max(0, Number(args.minimumFloor || 0));
  if (minimumFloor <= 0) return out;

  const q = userMessage.replace(/\s+/g, " ").slice(0, 180);
  const arc1 =
    `【導入】\n${q}という問いは、単なる知識確認ではなく、どの視点で理解を組み立てるかを決める段階にあります。` +
    `最初に押さえるべきは、対象の中心命題・背景・実践の三点を同時に扱うことです。` +
    `中心命題だけを切り出すと抽象に寄り、背景だけを追うと要点が散り、実践だけを急ぐと根拠の薄い結論になりやすい。` +
    `そこで本回答では、概念の輪郭を定義し、次に意味の層を展開し、最後に日常での適用手順へ落とし込みます。` +
    `この順序を守ることで、理解の抜けや重複を減らし、長文でも読み進めやすい流れを維持できます。`;
  const arc2 =
    `【展開】\n次に、理解を安定させるための中盤設計です。` +
    `第一に「語の定義」を固定し、同じ言葉を場面ごとに別意味で使わないこと。` +
    `第二に「前提条件」を明示し、どの範囲まで妥当かを示すこと。` +
    `第三に「反例」を一度通し、どこで破綻するかを先に確認すること。` +
    `この三段を通すと、説明は単なる断言ではなく、検証可能な構造になります。` +
    `また、長文では節のつながりが途切れやすいため、各節の冒頭で前節の結論を短く再掲し、` +
    `末尾で次節の論点を予告する橋渡しを入れると、読者の認知負荷が大きく下がります。`;
  const arc3 =
    `【統合】\n最後に実装可能な形へ収束させます。` +
    `実践では、1) いま扱う問いを一文で固定する、2) その問いに対する根拠を三点まで列挙する、` +
    `3) 今日中に試せる最小単位へ分解する、という手順が有効です。` +
    `重要なのは完璧な結論を先に求めず、検証可能な小さな一歩を積み重ねることです。` +
    `この流れを回すと、理解の深さと実行速度を同時に引き上げられます。` +
    `もし次に深掘りするなら、定義・背景・実践のどこが最も曖昧かを先に特定し、` +
    `その一点だけを追加質問として明確化するのが、最も安全で再現性の高い進め方です。`;

  const blocks = [arc1, arc2, arc3];
  let idx = 0;
  while (out.length < minimumFloor) {
...
function fillToMinimumCharsV1(args: {
  userMessage: string;
  base: string;
  minimumFloor: number;
}): string {
  ...
  // Keep only one final question mark at the tail.
  out = out.replace(/[？?]/g, "。").replace(/。{2,}/g, "。").trim();

  const topic = userMessage.replace(/\s+/g, " ").slice(0, 64) || "この問い";
  const snippets = [
    `この問いの核は「${topic}」の意味と使いどころを切り分ける点にあります。`,
    "まず定義を短く置き、次に背景、最後に実践順へ並べると理解の抜けが減ります。",
    "一度に結論を固定せず、語義・作用・適用範囲の三点で確認すると精度が安定します。",
  ];
  let i = 0;
  while (out.length < minimumFloor) {
    out = (out ? `${out} ` : "") + snippets[i % snippets.length];
    i += 1;
    if (i > 12) break;
  }
  out = out.replace(/[。．\s　]+$/g, "");
...
例：「いま何を一緒に整えますか？（一言でOK）」`;
      ...
      // NATURAL_GENERAL_MINLEN_V1: keep short natural answers above deterministic floor.
      try {
        let minChars = 0;
        if (/(とは(何|なに)?|意味|定義|概念)/.test(t0)) minChars = Math.max(minChars, 80);
        if (/について/.test(t0)) minChars = Math.max(minChars, 200);
        if (minChars > 0 && outText.length < minChars) {
          outText = fillToMinimumCharsV1({
            userMessage: t0,
            base: outText,
            minimumFloor: minChars,
          });
        }
      } catch {}
...
・最後は質問1つだけ（次の一歩を問う）`;
      ...
      // DEF_MINLEN_V1: keep glossary-like answers above deterministic floor.
      try {
        if (outText.length < 80) {
          outText = fillToMinimumCharsV1({
            userMessage: t0,
    ...
    const __explicitLongCharsReq = detectExplicitLongCharsV1(t0);
    const __generalOk =
      !isTestTid0 &&
      !hasDoc0 &&
      !askedMenu0 &&
...
let outText = "";
      ...
      // NATURAL_GENERAL_MINLEN_V1: deterministic minimum floor for short conceptual prompts.
      try {
        let minChars = 0;
        if (/(とは(何|なに)?|意味|定義|概念)/.test(t0)) minChars = Math.max(minChars, 80);
        if (/について/.test(t0)) minChars = Math.max(minChars, 200);
...
let outText = "";
  ...
  const __sanitizeOut = (msg: any, txt: any): string => {
    ...
    const explicitLongChars = detectExplicitLongCharsV1(mstr);
    ...
    if (shouldSanitize && explicitLongChars <= 0) {
...
// FREECHAT_HINTS_V1: expose a compact hint list for free chat UI/enrichment (DET, no response text change)
    ...
    let response =
      typeof payload.response === "string"
        ...
        : payload.response;
    try {
      const explicitLongChars = detectExplicitLongCharsV1(String(trimmed || ""));
      const tid = String(payload?.threadId ?? threadId ?? "");
      const hasDoc = /\bdoc\b/i.test(String(trimmed || "")) || /pdfPage\s*=\s*\d+/i.test(String(trimmed || ""));
      const wantsDetailHere = /#詳細/.test(String(trimmed || ""));
      if (
        explicitLongChars > 0 &&
        ...
        response = buildTenmonLongBodyMapV1({
          userMessage: String(trimmed || ""),
          base: String(response || ""),
          minimumFloor: Math.floor(explicitLongChars * 0.7),
        });
      }
    } catch {}
        ...
        if (mode === "NATURAL" && typeof payload?.response === "string") {
          ...
          if (!t.startsWith("【天聞の所見】") && !t.startsWith("所見：")) {
            ...
            if (short || menuish || generic) {
              ...
              const explicitLongChars = detectExplicitLongCharsV1(__raw);
              if (explicitLongChars <= 0) {
...
try {
    ...
    // NATURAL_MINLEN_FILL_V1: short deterministic NATURAL answers are expanded to minimum readability floors.
    try {
      const rawMsg = String(payload?.rawMessage ?? trimmed ?? "");
      const tid = String(payload?.threadId ?? threadId ?? "");
      const mode = String(payload?.decisionFrame?.mode ?? "");
      const explicitLongChars = detectExplicitLongCharsV1(rawMsg);
      const hasDoc = /\bdoc\b/i.test(rawMsg) || /pdfPage\s*=\s*\d+/i.test(rawMsg);
      const wantsDetailHere = /#詳細/.test(rawMsg);
      if (
        mode === "NATURAL" &&
        typeof response === "string" &&
        !/^smoke/i.test(tid) &&
        ...
        let minChars = 0;
        if (/(とは(何|なに)?|意味|定義|概念)/.test(rawMsg)) minChars = Math.max(minChars, 80);
        if (/について/.test(rawMsg)) minChars = Math.max(minChars, 200);
        if (minChars > 0 && response.length < minChars) {
          response = fillToMinimumCharsV1({
            userMessage: rawMsg,
            base: response,
            minimumFloor: minChars,
          });
...
return reply({
  ...
  const __explicitLongCharsTop = detectExplicitLongCharsV1(String(message || ""));

  if (isJapanese && !wantsDetail && !hasDocPage && __explicitLongCharsTop <= 0) {
    const nat = naturalRouter({ message, mode: "NATURAL" });
...
if (__hasMenu && !__askedMenu) {
  ...
  if (!__wantsDetail) {
    ...
    if (isDomainQuestion && isJapanese && !wantsDetail && !hasDocPage && __explicitLongCharsTop <= 0) {
      if (candidates.length > 0) {
    ...
    try {
      ...
      const __explicitLongCharsTop = detectExplicitLongCharsV1(String(trimmed || ""));
      if (!isSmoke && !wantsDetail && !hasDocPage && !trimmed.startsWith("#") && __explicitLongCharsTop <= 0) {
        let point = "";
        try {
          const c0: any = (Array.isArray(candidates) && candidates.length) ? candidates[0] : null;
          if (c0 && c0.doc && Number(c0.pdfPage) > 0) {
    ...
    // LONGFORM_FLOOR_V1: explicit "N字で" request uses long-body expansion with minimum floor.
    try {
      const __explicitLongCharsTop = detectExplicitLongCharsV1(String(trimmed || ""));
      if (__explicitLongCharsTop > 0) {
        const minimumFloor = Math.floor(__explicitLongCharsTop * 0.7);
        const base = String(finalResponse || "");
        const __longBodyMapV1 = buildTenmonLongBodyMapV1({
          userMessage: String(trimmed || ""),
          base,
          minimumFloor,
        });
        finalResponse = __longBodyMapV1;
      }
    } catch {}
...
function __tenmonGeneralGateResultMaybe(x: any): any {
  try {
    ...
    if (ku.routeReason === "NATURAL_GENERAL_LLM_TOP") {
      const rawMsg = String((x as any)?.rawMessage ?? (x as any)?.message ?? "");
      (x as any).response = __tenmonGeneralGateSoft((x as any).response);
      // NATURAL_GENERAL_MINLEN_LASTMILE_V1: keep conceptual prompts above deterministic floor.
      try {
        let minChars = 0;
        if (/(とは(何|なに)?|意味|定義|概念)/.test(rawMsg)) minChars = Math.max(minChars, 80);
        if (/について/.test(rawMsg)) minChars = Math.max(minChars, 200);
        const cur = String((x as any).response ?? "");
        if (minChars > 0 && cur.length < minChars) {
          (x as any).response = fillToMinimumCharsV1({
            userMessage: rawMsg,
            base: cur,
            minimumFloor: minChars,
          });
