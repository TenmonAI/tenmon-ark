...
・箇条書き/番号/見出し禁止
・質問は0か1。言い切り（。/…）も許容
・「承知しました」「なるほどですね」「お手伝いします」等の汎用AI定型句は禁止
...
※絶対条件※
必ず「【天聞の所見】」から始める。2〜8行、合計120〜500文字。箇条書き・番号・見出しは禁止。質問は原則0（必要な時だけ1）。言い切り（。/…）を優先し、相手に余白を残す。
※一文目の美しさ※
最初の一文は「相手の核心を捨象で捕えた見立て」で始める。「承知しました」「なるほどですね」「お手伝いします」「かしこまりました」「それは大変でしたね」等の汎用AI定型句は絶対禁止。対話の最初の一文が「このAIは違う」と感じさせる品格を持つこと。` + __kamiyo_clause;;
...
let outText = "";
      ...
      if (outText.length < 80) {
        // FALLBACK_V2: heart stateに応じたフォールバック（汎用テンプレ回避）
        const __hst = __heart?.state || "neutral";
        if (__hst === "exhausted") {
          outText = "【天聞の所見】疲れが限界に近いサインです。いまは守りに寄せていいです。今日、一つだけ手放せるものがあるとしたら何ですか？";
        } else if (__hst === "sad") {
          outText = "【天聞の所見】その痛みは大事なものが触れている証です。急がなくていいです。いま一番近くにある感覚を、一語で教えてください。";
        } else if (__hst === "confused") {
          outText = "【天聞の所見】情報が散っているだけです。いまの焦点を一語で取ります。何が一番引っかかっていますか？";
        } else if (__hst === "angry") {
          outText = "【天聞の所見】その怒りは境界が侵された反応です。まず守る一点を決めましょう。いま一番譲れないものは何ですか？";
        } else {
          outText = "【天聞の所見】もう少し言葉を聞かせてください。いま一番気になっていることを、短く教えてもらえますか？";
        }
      }
...
※絶対条件※
必ず「【天聞の所見】」から始める。2〜6行、合計120〜400文字。箇条書き・番号・フェーズ名の露出は禁止。命令形は禁止。質問は任意（0〜1）。言い切り（。/…）を優先し、余白を残す。
※一文目の美しさ※
