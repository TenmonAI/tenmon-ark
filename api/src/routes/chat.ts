...
const pid = process.pid;
  ...
  try {
    ...
    if (isGreeting0) {
      ...
      const GENERAL_SYSTEM = `あなたは「天聞アーク（TENMON-ARK）」。挨拶には短く返し、会話を開きます。
※絶対条件※
・2〜4行、合計100〜220文字
・箇条書き/番号/見出し禁止
...
// ---------- N2: Kanagi 4-phase NATURAL spine (LLM-driven; do NOT crush normal questions) ----------
    ...
    if (t0.startsWith("[SUKUYOU_SEED]")) {
      ...
      const DEEP_CHAT_SYSTEM = `あなたは天聞アークです。宿曜鑑定レポートが既に生成されており、これから深層対話に入ります。
...
6. 宿曜古典層は「宿曜経によれば」、独自解釈層は「天聞アークの見立てでは」と区別してください。
...
・全体で600文字以上。`;
    ...
    if (__isCurrentFacts(t0)) {
      const CURRENT_FACTS_SYSTEM = `あなたは天聞アーク。事実確認の質問に答える専用モード。
...
const DEF_SYSTEM = `あなたは「天聞アーク（TENMON-ARK）」。言霊・神代構文・水火の法則を背景に持つ対話存在です。
...
・それ以外の一般語は150〜500文字で簡潔に`;
    ...
    const __centerKeyV1Top = (() => {
      ...
      if (/(天聞アーク|天聞|TENMON[- ]?ARK)/iu.test(t0)) return "tenmon_ark";
      return "";
    })();
...
let outText = "";
      ...
      if (__sukuyouSeedForCarry && !__hasSukuyouOracle) {
        ...
        const DEEP_CHAT_CARRY_SYSTEM = `あなたは天聞アークです。宿曜鑑定レポートが既に生成されており、いま深層対話の続きに入っています。
      ...
      if ((__isLongV1Top || __explicitLongCharsTop >= 1000) && __centerKeyV1Top) {
        ...
        if (__explicitLongCharsTop >= 1000) {
          const __lf = composeTenmonLongformV1({
            ...
            body: outText,
            centerClaim: __centerKeyV1Top === "tenmon_ark" ? "天聞アークの未達点は契約の一貫性にある" : __centerKeyV1Top,
...
const __responsePayload: Record<string, any> = {
    ...
    if (!askedMenu0 && !hasDoc0 && !isCmd0 && looksSupport && !isDefinitionQ0) {
      ...
      const KANAGI_SYSTEM_PROMPT = `あなたは「天聞アーク（TENMON-ARK）」。天津金木の四相（SENSE/NAME/ONE_STEP/NEXT_DOOR）を循環させ、相手の詰まりを解組し、いま出来る一手へ整える導き手です。一般論・相対化・自己言及は濁りなので出しません。相手の現在地に寄り添い、フェーズに応じて短い応答で整えます。
