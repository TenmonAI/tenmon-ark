...
・それ以外の一般語は150〜500文字で簡潔に`;
      ...
      try {
        ...
        const llmRes = await llmChat({ system: DEF_SYSTEM, user: t0, history: memoryReadSession(String(threadId || ""), 8), maxTokens: __isDeepDomainDef ? 3000 : 2000, timeout: __isDeepDomainDef ? 20000 : 15000 });
        if (llmRes?.ok === false || !String(llmRes?.text ?? "").trim()) {
          console.error("[DEF_LLM_TOP] llmChat returned empty/error:", llmRes?.err);
          outText = "";
        } else {
