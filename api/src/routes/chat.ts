...
例：「いま何を一緒に整えますか？（一言でOK）」`;
    ...
    // TENMON_CLAMP_V3: 天津金木思考回路対応（文字数制限緩和、質問強制廃止）
    const __tenmonClampOneQ = (raw: string): string => {
      let t = String(raw ?? "").replace(/\r/g, "").trim();
      t = t.replace(/^\s+/, "").replace(/[ \t]+\n/g, "\n").replace(/\n{3,}/g, "\n\n").trim();
      t = t.replace(/^\s*\d+[.)].*$/gm, "").replace(/^\s*[-*•]\s+.*$/gm, "").trim();
      if (!t.startsWith("【天聞の所見】")) t = "【天聞の所見】" + t;
      // V3: 質問強制なし、文字数制限緩和（2000文字）
      if (t.length > 2000) t = t.slice(0, 2000).replace(/[。、\s　]+$/g, "") + "…";
      if (t.length < 40) t = "【天聞の所見】いま一番の焦点は何ですか？（一語でOK）";
      return t;
    };
      ...
      if (__hit) {
        ...
        return res.json(__tenmonGeneralGateResultMaybe({
          response: ((): string => {
            ...
            if (!t.startsWith("【天聞の所見】")) t = "【天聞の所見】" + t;
            // OLD_PIPELINE_RETIREMENT_V1: 260字clamp/？強制を除去、800字に緩和
            if (t.length > 800) t = t.slice(0, 800).replace(/[。、\s　]+$/g, "") + "…";
            return t;
          })(),
          evidence: null,
          candidates: [],
...
// TENMON_DEF_SYSTEM_V2: ドメイン用語の定義質問には知的資産を注入
const __defTermKnowledge = (() => {
  try {
    const __kCtx = buildKnowledgeContext(__rawDef);
    return __kCtx ? ("\n\n" + __kCtx + "\n\n【応答指示】上記の言灵秘書原典データを踏まえて定義を述べよ。音義・水火の性質・天津金木の運動パターンを具体的に引用せよ。") : "";
  } catch { return ""; }
})();
const __isDefDomain = /言霊|言灵|ことだま|コトダマ|カタカムナ|天津金木|あまつかなぎ|五十音|水火|イキ|正中|螺旋|神代|神器|フトマニ|音義|灵的|霊的|の灵|の霊|行の灵|火水|水火別|澄濁|五十連|十行|アーク|契約の箱|三種の神器|天地開闢|布斗麻邇|ヒフミヨイムナ|古事記|法華経|真言|曼荼羅|般若心経|天之御中主|国産み|ア行|カ行|サ行|タ行|ナ行|ハ行|マ行|ヤ行|ラ行|ワ行|母音|子音/i.test(__rawDef) ||
    /^[「『]?[ァ-ヶー][」』]?の(音義|意味|灵|霊|音|響き|力|性質)/.test(__rawDef) ||
    /^[「『]?[ぁ-ん][」』]?の(音義|意味|灵|霊|音|響き|力|性質)/.test(__rawDef) ||
    /^[「『]?[ァ-ヶー][」』]?(とは|って|を教えて|について)/.test(__rawDef);
// TENMON_DEF_DEEP_ANALYSIS_V3: 深層解析モード対応
const __needsDeep = __isDefDomain && needsDeepAnalysis(__rawDef);
const __needsSanskrit = needsSanskritAnalysis(__rawDef);
const __deepSuffix = __needsDeep
  ? ("\n\n" + DEEP_ANALYSIS_REPORT_INSTRUCTION + (__needsSanskrit ? "\n\nサンスクリット語源解読を【承】に必ず含めよ。音韻対応法則で各音を天津金木の運動パターンに還元せよ。" : ""))
  : "";
const DEF_SYSTEM = __isDefDomain
  ...
  : (TENMON_CONSTITUTION_TEXT + GENERAL_DEEP_THINKING_INSTRUCTION + `\n\n※絶対条件※\n必ず「【天聞の所見】」から始める。4〜8行、合計200〜500文字。箇条書き/番号/見出し禁止。\n天津金木思考回路で本質を見抜いて応答せよ。一般論や薄い共感ではなく、構造的真理に基づいた深い応答を返せ。\n最後は質問0〜1つ（必要な時だけ）。言い切り（。/…）を優先し、余白を残す。`);
    ...
    const __hasMidEmotionKw = /しんどい|しんどく|つらい|つらさ|苦しい|苦し[くみ]|不安|落ち込|凹[んむ]|へこ[んむ]|追い詰め|行き詰|息苦し|胸が[苦痛重]|心が[重折痛]|気持ちが[重沈暗]/.test(t0);
    const __hasDeepIntent = /どうしていい|どうすれば|わからない|整理したい|聞いてほしい|聞いて欲しい|寄り添|話を聞い|受け止め|向き合い|向き合う|抱え[きてる]|吐き出し|楽になりたい|逃げ出し|耐えられ|我慢でき/.test(t0);
  ...
  const __isDomainInGeneral = (
    ...
    /^[「『]?[ァ-ヶー][」』]?(とは|って|を教えて|について)/.test(t0)
  );

  // TENMON_KNOWLEDGE_CONTEXT_GENERAL_V1: ドメイン質問時は知的資産を動的注入
  let __domainKnowledgeClause = "";
  if (__isDomainInGeneral) {
    try {
      const __kCtx = buildKnowledgeContext(t0);
      if (__kCtx) {
...
必ず「【天聞の所見】」から始め、続けて御神託レポート全8章を構造化して提示せよ。` + __sukuyouContextClause;
      ...
      // TENMON_DOMAIN_GATE_BYPASS_V1: ドメイン質問時は濁りゲートを緩和
      if (!__isDomainInGeneral) {
...
return reply({
  ...
  // TENMON_DOMAIN_DETECT_V2: expanded domain detection (single-sound, row, kotodama, katakamuna, etc.)
  const isDomainLike = (
    /言霊|言灵|ことだま|コトダマ|カタカムナ|天津金木|あまつかなぎ|古事記|法華経|真言|布戸麻遮|フトマニ|水穂伝|虚空蔵|いろは|五十音|水火|イキ|正中|螺旋|神代|国産み|伊邪那岐|伊邪那美|神器|フトマニ図|曼荼羅|般若心経|天之御中主|淤能碁呂|音義|音の意味|灵的|霊的|行の灵|行の性質|火水|水火別|澄濁|五十連|十行|ア行|カ行|サ行|タ行|ナ行|ハ行|マ行|ヤ行|ラ行|ワ行|母音|子音|濁音|アーク|契約の箱|三種の神器|天地開闢|布斗麻邇|ヒフミヨイムナ/i.test(message) ||
    /^[「『]?[ァ-ヶー][」』]?の(音義|意味|灵|霊|音|響き|力|性質)/.test(message) ||
    /^[「『]?[あ-ん][」』]?の(音義|意味|灵|霊|音|響き|力|性質)/.test(message) ||
    /^[「『]?[ァ-ヶー][」』]?(とは|って|を教えて|について)/.test(message)
  );
  ...
  // TENMON_GUEST_DOMAIN_UNLOCK_V1: ドメイン質問はゲストでもHYBRID_LLMを許可
  if (shouldBlockLLMChatForGuest && shouldLLMChat && !isLocalTestBypass && !__isCard1Flow && !isFounder && !isDomainLike) {
    return res.status(200).json({
   ...
   if (shouldLLMChat) {
    ...
    if (isDomainLike) {
      try {
        ...
        // TENMON_KNOWLEDGE_INJECT_V1: 知的資産データを動的注入
        const knowledgeCtx = buildKnowledgeContext(trimmed);
        kanagiContext = [
          "",
          "【天津金木思考回路の解析結果】",
          `水火バランス: ${ikiDesc}`,
          ...
          "",
          "上記の解析結果と言灵秘書原典データを踏まえ、構文原理（言灵・水火・天津金木）に基づいて深く応答せよ。",
          "音の運動パターンと言灵秘書の音義から意味を読み解き、詩・論文・祈り・設計図が一体化した響きで語れ。",
          "各音の灵的意味を具体的に述べよ。「無にして有」「煤火」「昇水」「正火」等の原典用語を使え。",
        ].filter(Boolean).join("\n");
        llmMode = "HYBRID_LLM";
      } catch (__e) {
...
// TENMON_GENERAL_GATE_SOFT_V3: 天津金木思考回路対応（文字数・行数制限緩和、質問強制廃止）
function __tenmonGeneralGateSoft(out: string): string {
  ...
  // V3: 濁りフレーズのみクランプ。文字数・行数・質問数の制限は緩和。
  if (hasBad) {
    // keep content; remove bad phrases
    let u = String(t || "").replace(/\r/g, "").trim();

    // remove bullet/numbered lines
    u = u.replace(/^\s*\d+[.)].*$/gm, "").replace(/^\s*[-*•]\s+.*$/gm, "").trim();

    // keep at most 10 non-empty lines (V3: 緩和)
