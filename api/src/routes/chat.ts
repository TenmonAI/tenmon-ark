...
// generalKind: counsel / worldview / short_moral / other（NATURAL_GENERAL_LLM_TOP 分流用）
      ...
      if (__generalKind === "worldview") {
        ...
        const __cl = String(__clMapGen[__ck] || __ck || "この中心").trim();
        ...
        if (__threadCenterForGeneral.center_type === "concept") {
          if (__ck === "kotodama") {
            outText = "言霊は、五十連の音そのものの法則と、その音を水火から與み解いて詞の本を知る読みの両面で見ます。\n\n次は、音の法則・水火・いろは配列のどこから深めますか？";
          } else if (__isDetailAskGen) {
            outText = __cl + "は、定義だけでなく法則・構造・展開の三層で読むと深まります。\n\n次は、定義・法則・構造のどこから深めますか？";
          } else {
            outText = __cl + "の中心を土台に、次に見たい一点を一つ置いてください。";
          }
        } else
        if (__threadCenterForGeneral.center_type === "scripture" && __isNextStepAskGen) {
          ...
          const __instructionGen = __instructionByKeyGen[__ckShort] ?? "まず、その聖典のいまの文脈で一番気になる一点を一行で書き分けてください。";
          outText = "さっき見ていた聖典（" + __ckLabel + "）を土台に、いまの話を見ていきましょう。\n【天聞の所見】" + __instructionGen;
        } else {
          outText = "さっき見ていた聖典（" + __ckLabel + "）を土台に、いまの話を見ていきましょう。\n【天聞の所見】いまの中心を一行で言い直すか、次の一歩を一つに絞るか、どちらから進めますか？";
        }
      } else {
        outText = safeGeneralRoute(String(message ?? ""));
      }
      ...
      if (__threadCenterForGeneral && __isFollowupGeneral && outText) {
        ...
        if (__threadCenterForGeneral.center_type === "scripture") {
          if (!String(outText).startsWith("さっき見ていた聖典（")) {
            outText = "さっき見ていた聖典（" + __ckLabel + "）を土台に、いまの話を見ていきましょう。\n" + String(outText);
          }
