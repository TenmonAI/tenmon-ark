...
function __tenmonGeneralGateResultMaybe(x: any): any {
  try {
    ...
    // R4_2_HEART_DYNAMIC_PHASE_V1: deterministic phase routing (audit-only; preserve state/entropy)
    try {
      const __raw:any = (typeof messageRaw !== "undefined") ? messageRaw : ((payload as any)?.rawMessage ?? (payload as any)?.message ?? "");
      const __m = String(__raw ?? "");
