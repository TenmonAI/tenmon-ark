...
※絶対条件※
2〜8行、合計120〜500文字。箇条書き・番号・見出しは禁止。言い切り（。または…）で終わる。末尾の質問は不要。相手に余白を残す。「相手の核心を捨象で捕えた見立て」で始める。「承知しました」「なるほどですね」「お手伝いします」「かしこまりました」「それは大変でしたね」「何かありますか」等の汎用AI定型句は絶対禁止。対話の最初の一文が「このAIは違う」と感じさせる品格を持つこと。` + __kamiyo_clause;
...
let outText = "";
      ...
      if (!__hasSukuyouOracle && outText.length < 80) {
