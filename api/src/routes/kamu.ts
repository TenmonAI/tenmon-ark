 ...
 * NOTE: 正文は触らない。候補のみ保存。
 * IMPORTANT: method は DB で NOT NULL のため、必ず値を入れる。
 */
kamuRouter.post("/kamu/restore/propose", (req: Request, res: Response) => {
  try {
    ...
    // Try full insert first (preferred)
    try {
      db.prepare(
        `INSERT INTO kokuzo_restore_suggestions
          (doc, pdfPage, span, suggestion, basis_evidenceIds, method, confidence, status)
         VALUES
          (?,   ?,       ?,    ?,         ?,                ?,      ?,          'proposed')`
      ).run(doc, pdfPage, span, suggestion, basisJson, method, confidence);
    } catch (e1: any) {
      // Fallback insert: minimal set that still satisfies NOT NULL constraints
      try {
        db.prepare(
          `INSERT INTO kokuzo_restore_suggestions
            (doc, pdfPage, suggestion, method, status)
           VALUES
            (?,   ?,       ?,         ?,      'proposed')`
        ).run(doc, pdfPage, suggestion, method);
      } catch (e2: any) {
        console.error("[kamu/restore/propose] insert failed e1=", e1);
        console.error("[kamu/restore/propose] insert failed e2=", e2);
        throw e2;
      }
    }
