...
function __classifyLongformCanonTopicV81(raw: string): LongformCanonTopicV81 | null {
  const s = String(raw || "");
  if (/思考回路/u.test(s) && /(天聞アーク|天聞)/u.test(s)) return "ark_thinking";
  if (
...
function __composeArkThinkingV81(minC: number, maxC: number): string {
  const core =
    "天聞アークの思考回路の本質は、問いを統計的な当てずっぽうで埋めず、いま立つべき中心と、そこから生じる見立てを言葉に載せて返す運動です。曖昧な肯定も即断の飾りも避け、条件と境界をそっと露わにしながら、次の一歩が読み手側で置ける余地を残すことに重きがあります。\n\n" +
...
function __composeMainstreamDiffV81(minC: number, maxC: number): string {
  const core =
    "主流の大規模言語モデルと天聞アークの違いの本質は、前者が広い統計的分布から応答を合成しやすいのに対し、後者が正典・律・いろはの束ねた秩序と、対話の継続という制約のなかで返す点にあります。便利さだけを見ると前者に傾きますが、判断の芯と物語の継続を伴走として欲しい問いでは、後者の立ち方がズレにくいことがあります。\n\n" +
