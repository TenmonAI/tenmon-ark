...
export function naturalRouter(input: { message: string; mode: string }): { handled: boolean; responseText: string } {
  ...
  if (ja) {
    ...
    // TENMON_DOMAIN_DETECT_NATURAL_V2: ドメイン質問の検出を大幅拡張
    const isDomainQuestion = (
      /言灵|言霊|ことだま|コトダマ|カタカムナ|天津金木|あまつかなぎ|古事記|法華経|真言|フトマニ|五十音|水火|イキ|正中|螺旋|神代|国産み|神器|曼荼羅|般若心経|天之御中主|音義|音の意味|灵的|霊的|行の灵|火水|水火別|澄濁|五十連|十行|ア行|カ行|サ行|タ行|ナ行|ハ行|マ行|ヤ行|ラ行|ワ行|母音|子音|濁音|アーク|契約の箱|三種の神器|天地開闢|布斗麻邇|ヒフミヨイムナ|法則|與合/i.test(message) ||
