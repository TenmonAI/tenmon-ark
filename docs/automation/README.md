...
- **Apply lane**: `R8_KANAGI_SELF_KERNEL_V1` は `enabled: false`。`TENMON_APPLY_ENGINE` 未接続時は apply は実行されない（skip 扱い）
...
- **Env**: `automation/systemd/tenmon-auto-runner.env`（`TENMON_APPLY_ENGINE` は未設定のまま可）
